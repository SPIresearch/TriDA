...
def data_load(args): 
    ...
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=True, pin_memory=True)
    dsets["source_te"] = ImageList(te_txt, transform=image_test())
    dset_loaders["source_te"] = DataLoader(dsets["source_te"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=True, pin_memory=True)
    dsets["test"] = ImageList(txt_test, transform=image_test())
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=train_bs*2, shuffle=True, num_workers=args.worker, drop_last=False, pin_memory=True)
    args.num_samples  = len(dsets["source_tr"])
    return dset_loaders
...
def train_source(args):
    ...
    # for k, v in domain_discri.named_parameters():
    #     param_group += [{'params': v, 'lr': learning_rate}]
    ...
    while iter_num < max_iter:
        ...
        if iter_num % len(dset_loaders["source_tr"]) == 0:
        ...
        inputs_source = inputs_source.cuda(non_blocking=True)
        labels_source =  labels_source.cuda(non_blocking=True)
        inputs_imgnet = inputs_imgnet.cuda(non_blocking=True)
        target_imgnet = target_imgnet.cuda(non_blocking=True)


        inputs = torch.cat([inputs_source, inputs_imgnet], dim=0)
        feats = netB(netF(inputs))
        feats_source, feats_imgnet =torch.chunk(feats,2)
        
        outputs_source = netC(feats_source)
        outputs_imgnet = netFC(feats_imgnet)

        #domain_loss = domain_adv(feats_source, feats_imgnet)
        ...
        loss = classifier_loss + 0.3*imgcls_loss
    ...
    torch.save(best_netFC, osp.join(args.output_dir_src, "source_FC.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--select_cls', type=int, default=100)
    ...
    parser.add_argument('--output', type=str, default='ckps_imgnet/source_ce_d_100')
