...
def data_load(args): 
    ...
    # dsets["imgnet"] = ImageList_idx(txt_imgnet, transform=image_train())
    # dset_loaders["imgnet"] = DataLoader(dsets["imgnet"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=True, pin_memory=True)
...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        inputs_mixed = lam * inputs_imgnet + (1 - lam) * inputs_test
        inputs = torch.cat([inputs_test, inputs_imgnet, inputs_mixed], dim=0)
        ...
        mix_loss = lam * nn.CrossEntropyLoss()(outputs_imgnet_mix, target_imgnet) +  (1 - lam) * nn.CrossEntropyLoss()(outputs_source_mix, pred)
        mix_reg = nn.L1Loss(reduction='mean') (lam * feats_imgnet + (1 - lam) * feats_test, feats_mixed)
        mixloss = mix_loss + mix_reg
        ...
        classifier_loss+= 0.3*(imgcls_loss) + 0.1*mixloss
        optimizer.zero_grad()
    ...
    if args.issave:   
        ...
        torch.save(best_netFC, osp.join(args.output_dir, "target_FC_" + args.savename + ".pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='ckps_imgnet/target_ce_d_100')
    parser.add_argument('--output_src', type=str, default='ckps_imgnet/source_ce_d_100')
